...
class AuthManager:
    ...
    def save_credentials(
        ...
        enrollment_callsign: str | None = None, enrollment_color: str | None = None,
        enrollment_role: str | None = None
    ) -> None:
        """
        Save login hash and server address.
        Uses provided salt or generates new one.
        Also saves optional enrollment configuration fields.
        """
        ...
        data = {
            ...
            "salt": salt,
            "server": server,
            "enrollment_callsign": enrollment_callsign or "",
            "enrollment_color": enrollment_color or "",
            "enrollment_role": enrollment_role or "",
        }
    ...
    async def enroll(self, server: str, username: str, password: str) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                # Extract TAK configuration fields from server response
                enrollment_callsign = None
                enrollment_color = None
                enrollment_role = None
                for name, value in name_entries:
                    if name == "callsign":
                        enrollment_callsign = value
                    elif name == "color":
                        enrollment_color = value
                    elif name == "role":
                        enrollment_role = value
oid_map = {
                ...
                # Extract TAK config from sign response XML
                enrollment_callsign_from_sign = None
                enrollment_color_from_sign = None
                enrollment_role_from_sign = None
                for child in root:
                    tag_name = child.tag
                    if isinstance(tag_name, str) and "}" in tag_name:
                        tag_name = tag_name.split("}")[1]
                    ...
                    if tag_name == "signedCert":
                        ...
                        # Check for config fields in non-standard elements
                        if hasattr(child, "text") and child.text:
                            text = str(child.text or "")
                            if "callsign" in text.lower() or "color" in text.lower() or "role" in text.lower():
                                # Could be in a separate element, try to extract
                                pass

                # Also check root-level attributes or direct children
                for child in root:
                    if child.tag == "callsign":
                        enrollment_callsign_from_sign = child.text or None
                    elif child.tag == "color":
                        enrollment_color_from_sign = child.text or None
                    elif child.tag == "role":
                        enrollment_role_from_sign = child.text or None
if not raw_private_key:
    ...
    def get_enrollment_callsign(self) -> str | None:
        """Get the enrollment callsign from stored credentials."""
        try:
            with open(self.creds_file, encoding="utf-8") as f:
                data = json.load(f)
            return data.get("enrollment_callsign", "")
        except (OSError, json.JSONDecodeError):
            return None

    def get_enrollment_color(self) -> str | None:
        """Get the enrollment color from stored credentials."""
        try:
            with open(self.creds_file, encoding="utf-8") as f:
                data = json.load(f)
            return data.get("enrollment_color", "")
        except (OSError, json.JSONDecodeError):
            return None

    def get_enrollment_role(self) -> str | None:
        """Get the enrollment role from stored credentials."""
        try:
            with open(self.creds_file, encoding="utf-8") as f:
                data = json.load(f)
            return data.get("enrollment_role", "")
        except (OSError, json.JSONDecodeError):
            return None
